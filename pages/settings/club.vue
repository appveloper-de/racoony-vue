<template>
  <v-container fluid>
    <v-card>
      <v-card-title>
        <div class="headline">Vereinsdaten</div>
      </v-card-title>
      <v-divider></v-divider>
      <v-card-text>
        <v-container fluid>
      ...
      <v-divider></v-divider>
      <v-card-actions>
        <v-btn flat class="green--text darken-1" @click.native="storeSettings">Speichern</v-btn>
      </v-card-actions>
    </v-card>
  </v-container>
</template>
...
<script>
  export default {
    ...
    methods: {
      storeSettings () {
        this.loading = true
        return store.dispatch('settings/club_general/store')
          .then((response) => {
            this.loading = false
